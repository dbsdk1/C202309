...
int main() {
	char tasks[MAX_TASKS][CHAR_NUM] = { "" }; // 할 일 목록 저장을 위한 2차원 배열
	...
	while (1) {
		...
		case 1:
			...
			scanf_s("%s", tasks[taskCount], (int)sizeof(tasks[taskCount]));  // task[taskCount]에 할 일 저장(taskCount는 0부터 들어감), 사이즈도 써주기
			printf("할 일 ""%s""가 저장되었습니다\n\n", tasks[taskCount]);  // task[taskCount]에 저장된 문자열 출력
		...
		case 2:
			...
			if (delIndex > taskCount || delIndex <= 0) {  // 만약 지울 인덱스번호가 할 일 수보다 많거나 0이랑 같거나 작으면(즉, 벗어난 경우)
				...
				// 문자열 복사 함수로 삭제(조사한 결과 형식은 배열이름, 배열크기, "새로운 문자열" 이다.)
				strcpy_s(tasks[delIndex - 1], sizeof(tasks[delIndex - 1]), "");  // 마찬가지로 tasks는 0번부터 되어있기 때문에 delIndex에서 1을 빼줘야한다.)
				// 입력한 번호의 할 일 삭제 후 뒤에 있는 할 일을 앞으로 옮김.
				for (int i = delIndex; i < taskCount + 1; i++) {
					strcpy_s(tasks[i - 1], sizeof(tasks[i]), tasks[i]);  // 문자열을 정의된 문자배열에 넣을 수 있게함
				}
